...
// --- NOVI MODUL: HERO LABORATORIJ (2.5D INTERAKCIJA) ---
const HeroLaboratorij = () => {
    const [pours, setPours] = useState<string[]>([]);
    const [animatingTube, setAnimatingTube] = useState<string | null>(null);

    // Funkcija za izračun miješane boje na temelju tradicionalnog RYB modela (slikarstvo)
    const getMixedColor = (pourArray: string[]) => {
        if (pourArray.length === 0)
            return {
                hex: "transparent",
                name: "Prazna posuda",
                text: "slate-500",
        ...
        const counts = { r: 0, y: 0, b: 0 };
        pourArray.forEach((c) => counts[c as keyof typeof counts]++);
        const { r, y, b } = counts;

        // 3 iste
        if (r === 3)
            return { hex: "#EF4444", name: "Čista Crvena", text: "white" };
        if (y === 3)
            return { hex: "#FBBF24", name: "Čista Žuta", text: "slate-900" };
        if (b === 3)
            return { hex: "#3B82F6", name: "Čista Plava", text: "white" };

        // Miješanje 3 različite (sve primarne)
        if (r === 1 && y === 1 && b === 1)
            return {
                hex: "#78716C",
                name: "Smeđa / Siva (Neutralna)",
                text: "white",
            };

        // Dominacija jedne boje (2) + 1 druge
        if (r === 2 && y === 1)
            return {
                hex: "#EA580C",
                name: "Cinober (Crveno-narančasta)",
                text: "white",
            };
        if (r === 1 && y === 2)
            return { hex: "#F97316", name: "Narančasta", text: "white" };

        if (y === 2 && b === 1)
            return { hex: "#84CC16", name: "Žuto-zelena", text: "slate-900" };
        if (y === 1 && b === 2)
            return { hex: "#10B981", name: "Smaragdno zelena", text: "white" };

        if (b === 2 && r === 1)
            return { hex: "#6366F1", name: "Indigo plava", text: "white" };
        if (b === 1 && r === 2)
            return {
                hex: "#A855F7",
                name: "Purpurna / Ljubičasta",
                text: "white",
            };

        // Prijelazni koraci (1 ili 2 klika prije punjenja)
        if (r > 0 && y === 0 && b === 0)
            return {
                hex: "#EF4444",
                name: "Crvena (u nastajanju)",
                text: "white",
            };
        if (y > 0 && r === 0 && b === 0)
            return {
                hex: "#FBBF24",
                name: "Žuta (u nastajanju)",
                text: "slate-900",
            };
        if (b > 0 && r === 0 && y === 0)
            return {
                hex: "#3B82F6",
                name: "Plava (u nastajanju)",
                text: "white",
            };
        ...
        if (r === 1 && y === 1)
            return {
                hex: "#F97316",
                name: "Narančasta (u nastajanju)",
                text: "white",
            };
        if (y === 1 && b === 1)
            return {
                hex: "#22C55E",
                name: "Zelena (u nastajanju)",
                text: "white",
            };
        if (b === 1 && r === 1)
            return {
                hex: "#8B5CF6",
                name: "Ljubičasta (u nastajanju)",
                text: "white",
            };

        return { hex: "#333333", name: "Miješanje...", text: "white" };
    };

    const currentColor = getMixedColor(pours);
    const isFull = pours.length >= 3;

    const handlePour = (colorCode: string) => {
        if (isFull || animatingTube) return;
        setAnimatingTube(colorCode);

        // Nakon što animacija lijevanja završi, dodajemo boju u posudu
        setTimeout(() => {
            setPours((prev) => [...prev, colorCode]);
            setAnimatingTube(null);
        }, 600);
    };

    const resetVat = () => setPours([]);

    return (
        <div
            className='relative w-full overflow-hidden bg-slate-950 border-b border-slate-800 flex flex-col justify-center items-center shadow-2xl'
            style={{ height: "70vh", minHeight: "600px" }}>
            {/* 1. POZADINSKI SLOJ: Svjetlost (Prizma) i Tisak (CMYK) */}
            <div className='absolute inset-0 z-0 flex justify-between px-4 md:px-12 lg:px-24 pt-10 opacity-80 pointer-events-none'>
                {/* Lijevo: Fizika svjetlosti (Prizma) */}
                <div className='w-1/3 flex flex-col items-start relative hidden sm:flex'>
                    <div className='bg-slate-900/60 backdrop-blur-md border border-slate-700/50 p-4 rounded-xl shadow-lg max-w-xs mb-6'>
                        <h3 className='text-yellow-400 font-bold flex items-center gap-2 mb-2 text-sm uppercase tracking-wider'>
                            <Sun size={16} /> Svjetlost (Fizika)
                        </h3>
                        <p className='text-xs text-slate-400 leading-relaxed'>
                            Bijela svjetlost se lomi kroz prizmu stvarajući
                            spektar boja. Zbrajanjem RGB svjetala nastaje bijela
                            (<strong>Aditivno miješanje</strong>).
                        </p>
                    </div>
                    {/* Ilustracija Prizme */}
                    <div className='relative w-48 h-32 ml-8'>
                        <div className='absolute top-0 -left-20 w-32 h-2 bg-gradient-to-r from-white/0 to-white/80 rotate-[20deg] origin-right blur-[1px]'></div>
                        {/* Prizma Trokut */}
                        <svg
                            viewBox='0 0 100 100'
                            className='absolute top-10 left-10 w-20 h-20 drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]'>
                            <polygon
                        ...
                        {/* RGB Spektar koji izlazi */}
                        <div className='absolute top-16 left-28 w-40 h-10 bg-gradient-to-b from-red-500 via-green-500 to-blue-500 rotate-[-10deg] origin-left opacity-60 blur-sm rounded-r-full'></div>
                    </div>
                </div>

                {/* Desno: Tisak i CMYK */}
                <div className='w-1/3 flex flex-col items-end relative hidden sm:flex'>
                    <div className='bg-slate-900/60 backdrop-blur-md border border-slate-700/50 p-4 rounded-xl shadow-lg max-w-xs mb-6 text-right'>
                        <h3 className='text-cyan-400 font-bold flex items-center justify-end gap-2 mb-2 text-sm uppercase tracking-wider'>
                            CMYK (Tisak) <Printer size={16} />
                        </h3>
                        <p className='text-xs text-slate-400 leading-relaxed'>
                            Pigmenti upijaju svjetlost. Papir odbija samo ono
                            što nije upijeno. Zbrajanjem nastaje crna (
                            <strong>Suptraktivno miješanje</strong>).
                        </p>
                    </div>
                    {/* Ilustracija Printera koji printa miješanu boju */}
                    <div className='relative w-40 h-40 mr-12 mt-4 flex flex-col items-center'>
                        {/* Izlazni papir s trenutnom bojom */}
                        <div
                            className='absolute top-8 w-24 bg-white border border-slate-200 shadow-xl z-10 transition-all duration-1000 flex flex-col'
                            style={{
                                height: pours.length > 0 ? "100px" : "20px",
                            }}>
                            <div
                                className='w-full flex-grow transition-colors duration-1000'
                                style={{
                                    backgroundColor:
                                        currentColor.hex !== "transparent"
                                            ? currentColor.hex
                                            : "white",
                                }}></div>
                        ...
                        {/* Tijelo Printera */}
                        <div className='absolute top-0 w-32 h-12 bg-slate-800 border-2 border-slate-600 rounded-t-xl z-20 flex justify-center items-center'>
                            <div className='w-16 h-1 bg-cyan-500/50 rounded-full shadow-[0_0_10px_#06b6d4]'></div>
                        </div>
                    </div>
                </div>
            </div>

            {/* 2. PREDNJI SLOJ: Interaktivni stol s epruvetama */}
            <div className='relative z-10 flex flex-col items-center mt-20'>
                {/* Naslov u centru */}
                <div className='text-center mb-16 px-4'>
                    <h1 className='text-4xl md:text-5xl lg:text-6xl font-black mb-4 tracking-tight drop-shadow-xl'>
                        Sinteza boja:{" "}
                    ...
                    <p className='text-lg text-slate-300 max-w-xl mx-auto'>
                        Kreirajte povijesne nijanse u digitalnom laboratoriju.
                        Pomiješajte točno 3 doze primarnih boja za savršen ton.
                    </p>
                </div>

                {/* Stol */}
                <div className='relative flex justify-center items-end h-64 w-[600px] max-w-full perspective-1000'>
                    {/* Centralna posuda (Vat) */}
                    <div className='relative w-40 h-48 border-4 border-slate-600/50 bg-slate-800/30 backdrop-blur-md rounded-b-[40px] rounded-t-lg mx-8 flex flex-col justify-end overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] z-20 transition-all duration-500'>
                        {/* Tekućina u posudi */}
                        <div
                            className='w-full transition-all duration-1000 ease-in-out relative flex items-center justify-center'
                            style={{
                                height: `${(pours.length / 3) * 100}%`,
                                backgroundColor: currentColor.hex,
                                boxShadow:
                                    pours.length > 0
                                        ? `inset 0 10px 20px rgba(0,0,0,0.3), 0 0 30px ${currentColor.hex}40`
                                        : "none",
                            }}>
                            {/* Refleksija tekućine */}
                        ...
                        {/* Info pločica preko posude kada je puna */}
                        {isFull && (
                            <div className='absolute inset-0 flex flex-col justify-center items-center bg-black/40 backdrop-blur-sm animate-in fade-in zoom-in duration-500'>
                                <span
                                    className={`font-black text-xl text-center px-2 mb-1 text-${currentColor.text}`}
                                    style={{
                                        textShadow:
                                ...
                                <span
                                    className={`font-mono text-sm bg-black/50 px-2 py-1 rounded text-${currentColor.text}`}>
                                    {currentColor.hex}
                                </span>
                                <button
                                    onClick={resetVat}
                                    className='mt-4 bg-white/20 hover:bg-white/40 text-white p-2 rounded-full transition-colors backdrop-blur-md'>
                                    <RotateCcw size={20} />
                                </button>
                            </div>
                        )}
                    ...
                    {/* Tri Epruvete (Red, Yellow, Blue) */}
                    <div className='absolute w-full h-full flex justify-between px-10 items-start top-10 pointer-events-none'>
                        {/* Crvena Epruveta */}
                        <div className='relative pointer-events-auto'>
                            <button
                                onClick={() => handlePour("r")}
                                disabled={isFull || !!animatingTube}
                                className={`w-12 h-32 border-2 border-white/20 bg-white/5 backdrop-blur-sm rounded-b-full rounded-t-sm relative overflow-hidden transition-all duration-500 origin-bottom hover:-translate-y-2 hover:shadow-[0_0_20px_rgba(239,68,68,0.4)]
                  ${animatingTube === "r" ? "rotate-[60deg] translate-x-12 -translate-y-8" : ""}
                  ${isFull ? "opacity-50 grayscale" : "cursor-pointer"}
                `}>
                                <div className='absolute bottom-0 w-full h-20 bg-red-500 shadow-[inset_0_10px_10px_rgba(0,0,0,0.2)]'></div>
                                <div className='absolute top-0 left-2 w-1 h-full bg-white/30 rounded-full'></div>
                            </button>
                            {/* Kapljica animacija */}
                            {animatingTube === "r" && (
                                <div
                                    className='absolute top-10 right-[-30px] w-4 h-6 bg-red-500 rounded-b-full rounded-t-[50%] animate-bounce opacity-80'
                                    style={{
                        ...
                        {/* Žuta Epruveta (Sredina iza/iznad posude) */}
                        <div className='relative -top-16 pointer-events-auto z-10'>
                            <button
                                onClick={() => handlePour("y")}
                                disabled={isFull || !!animatingTube}
                                className={`w-12 h-32 border-2 border-white/20 bg-white/5 backdrop-blur-sm rounded-b-full rounded-t-sm relative overflow-hidden transition-all duration-500 origin-bottom hover:-translate-y-2 hover:shadow-[0_0_20px_rgba(250,204,21,0.4)]
                  ${animatingTube === "y" ? "rotate-[40deg] translate-x-8 translate-y-4" : ""}
                  ${isFull ? "opacity-50 grayscale" : "cursor-pointer"}
                `}>
                                <div className='absolute bottom-0 w-full h-20 bg-yellow-400 shadow-[inset_0_10px_10px_rgba(0,0,0,0.2)]'></div>
                                <div className='absolute top-0 left-2 w-1 h-full bg-white/30 rounded-full'></div>
                            </button>
                            {animatingTube === "y" && (
                                <div className='absolute top-20 right-[-10px] w-4 h-6 bg-yellow-400 rounded-b-full rounded-t-[50%] animate-bounce opacity-80'></div>
                            )}
                        </div>
                        ...
                        <div className='relative pointer-events-auto'>
                            <button
                                onClick={() => handlePour("b")}
                                disabled={isFull || !!animatingTube}
                                className={`w-12 h-32 border-2 border-white/20 bg-white/5 backdrop-blur-sm rounded-b-full rounded-t-sm relative overflow-hidden transition-all duration-500 origin-bottom hover:-translate-y-2 hover:shadow-[0_0_20px_rgba(59,130,246,0.4)]
                  ${animatingTube === "b" ? "-rotate-[60deg] -translate-x-12 -translate-y-8" : ""}
                  ${isFull ? "opacity-50 grayscale" : "cursor-pointer"}
                `}>
                                <div className='absolute bottom-0 w-full h-20 bg-blue-500 shadow-[inset_0_10px_10px_rgba(0,0,0,0.2)]'></div>
                                <div className='absolute top-0 left-2 w-1 h-full bg-white/30 rounded-full'></div>
                            </button>
                            {animatingTube === "b" && (
                                <div
                                    className='absolute top-10 left-[-30px] w-4 h-6 bg-blue-500 rounded-b-full rounded-t-[50%] animate-bounce opacity-80'
                                    style={{
                                        transform: "rotate(30deg)",
                                    }}></div>
                            )}
                    ...
                    {/* Vizualna linija stola */}
                    <div className='absolute bottom-0 w-full h-2 bg-gradient-to-r from-slate-900 via-slate-600 to-slate-900 rounded-full shadow-[0_10px_20px_rgba(0,0,0,0.8)]'></div>
                </div>

                {/* Helper tekst na dnu ekrana */}
                <p className='mt-8 text-sm text-slate-500 flex items-center gap-2 font-medium tracking-wide'>
                    <Info size={16} /> Kliknite na epruvete za dodavanje boje (
                    {pours.length}/3 dodano)
                </p>
            </div>
        </div>
    );
...
// --- NOVI MODUL: HERO RGB SINTEZA (ADITIVNO MIJEŠANJE SVJETLOSTI) ---
const HeroRGBSinteza = () => {
    const [r, setR] = useState(255);
    const [g, setG] = useState(255);
    const [b, setB] = useState(255);
    const [decompose, setDecompose] = useState(false);

    // Funkcija za pretvaranje decimalnog broja u dvostruki HEX string
    ...
    return (
        <div className='w-full bg-slate-950 border-b border-slate-800 pt-16 pb-20 relative overflow-hidden shadow-2xl'>
            <div className='max-w-6xl mx-auto px-4 md:px-8'>
                ...
                <div className='text-center mb-12 relative z-10'>
                    ...
                    <h1 className='text-4xl md:text-5xl lg:text-6xl font-black mb-6 tracking-tight'>
                        Svjetlost postaje boja{" "}
                        <span className='text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-green-500 to-blue-500'>
                            RGB
                        </span>
                    </h1>
                    <p className='text-lg text-slate-400 max-w-3xl mx-auto leading-relaxed'>
                        Svaka boja na ekranu računala stvara se miješanjem
                        različitih količina crvene, zelene i plave svjetlosti.
                        Zasloni se sastoje od tisuća sitnih kvadratića zvanih
                        pikseli. Oni su crni kada nisu aktivni, a kada piksel
                        ima sve tri boje u maksimalnoj vrijednosti, svjetlit će
                        bijelo.
                    </p>
                </div>

                {/* Glavni radni prostor (Canvas + Kontrole) */}
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 items-center'>
                    {/* Lijeva strana: Vizualizator (Canvas) */}
                    <div className='flex flex-col items-center'>
                        {/* Toggle za Decompose */}
                        <div className='mb-6 flex items-center gap-3 bg-slate-900 px-4 py-2 rounded-full border border-slate-700 z-10'>
                            <Layers size={18} className='text-slate-400' />
                            <label
                                htmlFor='decompose-toggle'
                                className='text-sm font-bold text-slate-300 cursor-pointer select-none'>
                                Razdvoji (Decompose R+G+B)
                            </label>
                            <button
                                id='decompose-toggle'
                                onClick={() => setDecompose(!decompose)}
                                className={`w-12 h-6 rounded-full transition-colors relative flex items-center px-1 ${decompose ? "bg-blue-600" : "bg-slate-700"}`}>
                                <div
                                    className={`w-4 h-4 bg-white rounded-full transition-transform duration-300 shadow-md ${decompose ? "translate-x-6" : "translate-x-0"}`}></div>
                            </button>
                        </div>

                        {/* Crni kontejner za mix-blend-screen */}
                        <div className='relative w-full max-w-md aspect-square bg-black rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-slate-800 flex justify-center items-center overflow-hidden'>
                            {/* Crveni krug */}
                            <div
                                className='absolute w-40 h-40 md:w-56 md:h-56 mix-blend-screen transition-all duration-1000 ease-in-out'
                                style={{
                                    backgroundColor: `rgb(${r}, 0, 0)`,
                                    transform: decompose
                                        ? "translate(-20%, -20%)"
                                        : "translate(-10%, -10%)",
                                    borderRadius: decompose
                                        ? "0 50% 50% 50%"
                                        : "50%",
                                }}></div>
                    ...
                    {/* Desna strana: Paneli s vrijednostima i slajderi */}
                    <div className='flex flex-col gap-6'>
                        {/* Finalni rezultat box */}
                        <div className='bg-slate-900 border border-slate-700 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-6'>
                            <div className='flex-grow'>
                                ...
                                <div className='font-mono text-xl text-white tracking-wide mt-1'>
                                    #
                                    <span className='text-red-400 font-bold'>
                                        {hexR}
                                    </span>
                            ...
                            <div
                                className='w-24 h-24 rounded-2xl shadow-inner border border-white/10 flex-shrink-0 transition-colors duration-200'
                                style={{
                        ...
                        <div className='bg-red-950/20 border border-red-900/50 p-5 rounded-2xl relative overflow-hidden'>
                            ...
                            <div className='flex justify-between items-end mb-4'>
                                <div>
                                    ...
                                    <div className='flex gap-4 text-sm font-mono text-red-200'>
                                        <span>
                                            Dec: <b>{r}</b>
                                        </span>
                                        <span>
                                ...
                                <div className='text-xs font-mono text-red-400/70 bg-red-950/50 px-2 py-1 rounded'>
                                    {Math.floor(r / 16)} * 16 + {r % 16}
                                </div>
                            </div>
                            <input
                                type='range'
                        ...
                        {/* GREEN Slajder Panel */}
                        <div className='bg-green-950/20 border border-green-900/50 p-5 rounded-2xl relative overflow-hidden'>
                            <div className='absolute left-0 top-0 w-1 h-full bg-green-500'></div>
                            <div className='flex justify-between items-end mb-4'>
                                <div>
                                    <span className='block text-green-500 font-bold mb-1'>
                                        Zelena (Green)
                                    </span>
                                    <div className='flex gap-4 text-sm font-mono text-green-200'>
                                        <span>
                                            Dec: <b>{g}</b>
                                        </span>
                                        <span>
                                ...
                                <div className='text-xs font-mono text-green-400/70 bg-green-950/50 px-2 py-1 rounded'>
                                    {Math.floor(g / 16)} * 16 + {g % 16}
                                </div>
                            </div>
                            <input
                                type='range'
                        ...
                        {/* BLUE Slajder Panel */}
                        <div className='bg-blue-950/20 border border-blue-900/50 p-5 rounded-2xl relative overflow-hidden'>
                            <div className='absolute left-0 top-0 w-1 h-full bg-blue-500'></div>
                            <div className='flex justify-between items-end mb-4'>
                                <div>
                                    <span className='block text-blue-500 font-bold mb-1'>
                                        Plava (Blue)
                                    </span>
                                    <div className='flex gap-4 text-sm font-mono text-blue-200'>
                                        <span>
                                            Dec: <b>{b}</b>
                                        </span>
                                        <span>
                                ...
                                <div className='text-xs font-mono text-blue-400/70 bg-blue-950/50 px-2 py-1 rounded'>
                                    {Math.floor(b / 16)} * 16 + {b % 16}
                                </div>
                            </div>
                            <input
                                type='range'
